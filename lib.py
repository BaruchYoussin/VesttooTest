...
class Arima_0_1_1(torch.nn.Module):
    ...
    Fields:
        ...
        tan_ma_coeff: tan(theta_1 * pi/2) where theta_1 is the first Moving Average coefficient in the doc.
        std_innovation: the std of the innovation process, sigma in the doc; it is allowed to be negative, as it is
            used only in the squared form.
    The initial value Y_0 is not included among the fields since it is not involved in learning but rather taken to be
    the initial value of the given time series.
    """
...
def _ma_matrix_numpy(ma_coeff: float, data_length: int) -> np.array:
    """Returns the matrix of the moving average transformation from innovations to their moving averages, A in the doc.

    :param ma_coeff: the coefficient of the MA(1) model, theta_1 in the doc.
...
def loss(model: Arima_0_1_1, time_block) -> torch.Tensor:
    ...
    :param model: The model being learned.
...
def prob_estimate(model: Arima_0_1_1, time_block: np.array):
    """Estimate of the probability of time_block being generated by the model, per MaxLikelihoodARIMA.odt"""
    assert len(time_block.shape) == 1
    length = time_block.shape[0] - 1  # n + 1 in the doc
    differences = torch.tensor(time_block, dtype=torch.float).diff()
    r = math.sqrt(_squares(differences, model.arma_const,
                           _cov_matrix(model.ma_coeff(), model.std_innovation, length)))
    sf = scipy.stats.norm.sf(r)  # Phi(-r) in the doc
    return 2 * length * sf if r > 4 else 1 - (1 - 2 * sf) ** length
